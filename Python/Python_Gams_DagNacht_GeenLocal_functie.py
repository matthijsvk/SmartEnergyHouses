## running gams files from Variabel_inclusief_local_energy
import gams
import os
def init():
    global t1
    ws=gams.GamsWorkspace(working_directory=os.getcwd(), debug=gams.DebugLevel.ShowLog)
    t1 = ws.add_job_from_file("Dagnachttarief_geen_local_realistisch.gms")
    t1.run()
...
def run():
    ...
    ## pconvlst: returns used power at given quarter (in Watt) without local energy!
...
## curtlst: returns the amount of curtailed energy - local produced energy that is not used
##for rec14 in t1.out_db["Curt"]:
##    curtlst+= [rec14.level]
