...
##############################################################
                                                             #
huidige_programma=  Variabel_Local## Choose your program     #
...
def update(timeValues, sensorValues, programListIDs, prices, boilerCons):
    ...
    for i in range(length):
        if (len(times)<1) or (timeValues[i] not in times):   
            ...
            if( timeValues[i] >= 7.75*60.0 and timeValues[i] <= 8.25*60.0):
                consumption += 0.20*getHouseConsumptionNotSmart(timeValues[i])
            if (timeValues[i] >= 17.75*60.0 and timeValues[i] <= 18.25*60.0) :
                consumption += 0.50*getHouseConsumptionNotSmart(timeValues[i])
            if( timeValues[i] >= 16.0*60.0 and timeValues[i] <= 17.25*60.0):
                consumption += 0.30*getHouseConsumptionNotSmart(timeValues[i])
            if timeValues[i] == 22.25*60.0:
                consumption += 0.30*getHouseConsumptionNotSmart(timeValues[i])
...
#Plot the final plot instantly
def instantPlot():
    category1 = []
    counter_ = 0
    for counter_ in range(0,96):
        ...
        update(thisTime,[[]],thisPrograms, thisPrices, thisBoilerCons)
    repaint()
####################################################################################################################
######################_______________#####################
#####################|Initialise GUI|#####################
...
if __name__ == '__main__':
    ...
    window.move(975+1400,500)
    window.show_all()
    window = builder.get_object("History")
    window.move(0+1400,475)
    window.show_all()
    window = builder.get_object("plotWindow")
    window.move(975+1180,0)
    window.show_all()
    window = builder.get_object("RunningPrograms")
    window.move(75+1400,0)
    window.show_all()
...
gobject.timeout_add(50, instantPlot)
