## running gams files from Variabel_geen_local_realstisch
import gams
import os
def init():
    global t1
    ws=gams.GamsWorkspace(working_directory=os.getcwd(), debug=gams.DebugLevel.ShowLog)
    t1 = ws.add_job_from_file("Variabel_geen_local_realistisch.gms")
    t1.run()
def run():
    ...
    for rec13 in t1.out_db["y"]:
        prtotlst+= [rec13.level]
